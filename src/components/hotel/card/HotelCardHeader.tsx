...
const HotelCardHeader: React.FC<HotelCardHeaderProps> = ({ hotel, useGridView }) => {
  ...
  return (
    <CardHeader className="pb-0 p-1 sm:p-2">
      <div className="flex flex-row items-center space-x-2">
        <motion.div 
          layoutId={`hotel-image-${hotel.id}`}
          className="h-8 w-8 sm:h-10 sm:w-10 overflow-hidden rounded-md flex-shrink-0"
        ...
        <div className="flex-1">
          ...
          <motion.div className="flex items-center mt-0.5 gap-1 flex-wrap" layoutId={`hotel-badges-${hotel.id}`}>
            <Badge variant="outline" className="uppercase text-[8px] sm:text-[9px] px-1 py-0 bg-indigo-50 dark:bg-indigo-900 text-indigo-600 dark:text-indigo-400 font-medium">
              {hotel.posKey}
            </Badge>
            {hotel.country && (
              <Badge variant="outline" className="text-[8px] sm:text-[9px] px-1 py-0 bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 font-medium">
                {hotel.country}
              </Badge>
            )}
          </motion.div>
